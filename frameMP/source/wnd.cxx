...
CWndBase::CWndBase() : m_hWnd(NULL), m_hWndPar(NULL)
{
...
CWndBase::CWndBase(const SWinInfo &info)
            : m_hWnd(info.m_hWnd), m_hWndPar(info.m_hPar), m_winInfo(info)
{
    if (s_InstCount == 1)
    {
        TrVoid(!Init());
...
double CWndBase::CmpScreenShot(LPCTSTR filePath)
{
    TrFu;
	return CmpScreenShotEx(filePath, SRect());
}

double CWndBase::CmpScreenShotEx(LPCTSTR filePath, const SRect& rc)
{
	TrVar(filePath);
	CPath path(filePath);
	TrRet(!path.IsFile(), 0.);
	...
	if (refFsize && GetScreenShot(pData, dataSize, CutWndRect(rc)) && frameMP::ReAlloc(&pRefBytes, 0, refFsize)
